// I found realization of maximum cost matching algorithm (but initial solution required maximal matching if min(max) cost)
// just make three vertices from one basket
// u - ball, v - basket 
// add edges (u , v1 , 4) (u , v2 , 2) (u, v3, 2) (v2, v3, 1) and run max cost matching
// max matching first will pair all balls and busket, then having  this edges (v2, v3, 1) will combine as much as possible basket with only two balls, to 
// one basket with three ball and one basket with one ball
...
void my_solve() {
    ...
    for (int i = 1; i <= m; ++i) {
        add_edge(basket[i][1], basket[i][2], 1);
    }
    ...
    for (int i = 1; i <= k; ++i) {
        ...
        add_edge(a, basket[b][0], 4);
        add_edge(a, basket[b][1], 2);
        add_edge(a, basket[b][2], 2);
    }
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        dout << i << " :  " << val << endl;
        for (int e = 1; e <= m; ++e) {
            if (val >= basket[e][0] && val <= basket[e][2]) {
                pos = e;
                break;
            }
        }
